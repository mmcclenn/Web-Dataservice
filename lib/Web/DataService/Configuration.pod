...
Define one or more output blocks using C<define_block>.  This can be done
either before or after the node definitions.
...
either in the main application file or in some subsidiary module, so that the
hierarchical relationships will be apparent.  Exactly how you structure your
applicaton is up to you.
...
change the external name (i.e. the name actually used in requests) by adding
C<=name>.  The name 'standard' produces a standard list (see below).  So the
following attribute value would enable the standard list except for 'datainfo'
and would set the external name of the 'header' parameter to 'head'.
...
data services.  One or the other mechanism will ensure that the proper service
is selected to respond to each request.
...
If this attribute is not specified, and if F<Template.pm> has already been
loaded, then the plugin L<Web::DataService::Plugin::TemplateToolkit> will be
loaded automatically.  If no templating plugin is loaded, then documentation
pages cannot be produced.  In that case, the features 'documentation' and
'doc_paths' will be automatically disabled if they were set.
...
The format names names listed below are predefined by Web::DataService and can
be activated simply by specifying a hashref with the appropriate C<name>
...
See below for descriptions of the various attributes.

=over

=head4 json

This output format expresses operation results in
L<JSON|https://en.wikipedia.org/wiki/JSON> (JavaScript Object Notation).  Its
content type is "application/json", and its default documentation node is
"formats/json".

=head4 txt

This output format expresses operation results as lines of L<comma-separated
...
"text/plain", and its default documentation node is "formats/txt".

=head4 csv

This output format is identical to L</txt>, except that its content type is
"text/csv".

=head4 tsv

This output format expresses operation results as lines of L<tab-separated
...
C<default_format>, and can be used to select the format either as a URL path
suffix or via the special parameter L</format> depending upon which data
service features are enabled.

This attribute is required for all feature definitions.

=head3 title

The value of this attribute is used as the format's title in documentation.
...
The special parameter C<header> will only be enabled if at least one format
uses it.  You can make sure this is so by providing this attribute with a true
value when defining a custom format that will use this parameter.  All of the
"text" builtin formats set this by default.
...
The first arguemnt to C<define_block> must be a string that provides the name
