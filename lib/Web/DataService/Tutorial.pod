...
Change the path prefix on line 60 of F<Example.pm>.
...
any field by adding C<name => 'my_new_name'> to its definition hash.
...
Under the Web::DataService framework, a data service application consists of the following list of components.
...
URL path.  In the simplest case, as shown in the L<example application|/lib/Example.pm> below, the route
handler reduces to the following:
...
The "meat" of your data service consists of subroutines that talk to the backend data store to fetch and/or
...
such as parameters, value sets, and output fields.  A typical data service application will contain several
role modules, one for each different kind of data to be returned.
...
Define all of the elements that will be referenced in data service node definitions which use this role.
...
You must also set up some means of selecting between the two data service instances, either differing path
prefixes or version selectors (see below).
...
case is to put each data service instance into a separate global variable (i.e. C<$ds1>, <$ds2>, <$ds3>, ... )
